            ...
            try {
                ...
                while ((line = buffReader.readLine()) != null) {
                    ...
                    if (isExempt) {
                        if (isImported) {
//                            tax = (price * 0.05);
                            double tmp = doubleRound(price * 5 / 100);
                            tax = doubleRound4tax(tmp);
                        }
                    } else {
                        if (isImported) {
//                            tax = (price * 0.15);
                            double tmp = doubleRound(price * 15 / 100);
                            tax = doubleRound4tax(tmp);
                        } else {
//                            tax = (price * 0.10);
                            double tmp = doubleRound(price * 10 / 100);
                            tax = doubleRound4tax(tmp);
                        }
                    }
                    sumTax += (tax);
...
//
        if (g >= 6) {
            d = (d*100 - g +10) /100;
            bg = new BigDecimal(d).setScale(2, RoundingMode.FLOOR);
            d = bg.doubleValue();
        }
//        BigDecimal bg = new BigDecimal(d).setScale(2, RoundingMode.UP);
//        return bg.doubleValue();
...
//        System.out.println(calTax.extractPrice(line1));
        ...
        System.out.println(calTax.doubleRound(0.5625));
